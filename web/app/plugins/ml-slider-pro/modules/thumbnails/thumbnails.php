...
class MetaSliderThumbnails {
    ...
    public function metaslider_flex_filmstrip_html($html, $slider_id, $settings) {
        if (isset($settings["navigation"]) && $settings['navigation'] == 'filmstrip') {
            ...
            $html .='<div id="metaslider_' . $slider_id . '_filmstrip" class="flexslider filmstrip">';
            $html .= "\n            <ul class='slides'>";
            ...
            while ($query->have_posts()) {
                ...
                if ($type == 'post_feed') {
                    ...
                    while ( $the_query->have_posts() ) {
                        ...
                        $html .= "\n                <li class=\"ms-thumb slide-{$query->post->ID} post-{$the_query->post->ID}\" style=\"display: none;\"><img src=\"{$url}\" /></li>";
                    }
                } else {
                    // generate thumbnail
                    $imageHelper = new MetaSliderImageHelper(
                    ...
                    if (strlen($url)) {
                        $html .= "\n                <li class=\"ms-thumb slide-{$query->post->ID}\" style=\"display: none;\"><img src=\"{$url}\" /></li>";
                    }
                }
            }
