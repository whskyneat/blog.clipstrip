...
class MetaLayerSlide extends MetaSlide {
    ...
    public function add_downscale_only_setting($aFields, $slider) {
        $newFields = array(
            'layer_scaling' => array(
                ...
                'label' => __("Layer Scaling", "metaslider"),
    ...
    protected function get_admin_slide() {
        ...
        $row .= "            <li rel='tab-3'>Extra</li>";
        $row .= "            <li rel='tab-4' class='editSource' data-editor='editor{$this->slide->ID}'>Edit Source</li>";
        $row .= "        </ul>";
    ...
    protected function get_public_slide() {
        ...
        add_action('metaslider_register_public_styles', array($this, 'enqueue_animate_css'));
    ...
    private function html_overlay() {
        ...
        return "<div class='msHtmlOverlay'{$slide_link}>" . do_shortcode($layer_html) . "</div>";
    }
    ...
    private function get_flex_slider_markup() {
        ...
        if (version_compare(METASLIDER_VERSION, 2.3, '>=')) {
            ...
            $html = "<li class='slider-{$this->slider->ID} slide-{$this->slide->ID} ms-layer' style='display: none;'{$thumb}>" . $html . "</li>";
        }
    ...
    /**
     * Enqueue the CSS Animation CSS file
     */
    public function enqueue_animate_css() {
    	wp_enqueue_style('metaslider-pro-animate', plugins_url( 'assets/animate/animate.css' , __FILE__ ), false, METASLIDER_VERSION);
    }
    ...
    public function get_responsive_javascript($javascript, $slider_id) {
        ...
        if ($this->settings['layer_scaling'] == 'down_only') {
            $downscale_only = "if (parseFloat(new_width) >= parseFloat(orig_width)) {
                            var multiplier = 1;
                            var percentage = 100;
                    }";
        }
        ...
        $html = "
            ...
            jQuery('#metaslider_{$slider_id} .msHtmlOverlay .layer[data-link], #metaslider_{$slider_id} .msHtmlOverlay[data-link]').each(function() {
